...
read -p "Indexul la care vreti sa efectuati modificarea este: " changeIndex

linePos=$((changeIndex+1))
linie=$(sed "${linePos}q;d" data.csv)

while true; do
    if [ -n "$linie" ]; then
        echo "Se va modifica urmatoarea linie: "
        echo $linie
        echo
        break 2;
    else
...
read -p "Daca doriti sa modificati PERMANENT aceasta linie din fisier, apasati tasta 1, sau orice alta tasta pentru a anula aceasta operatie de stergere: " choice
